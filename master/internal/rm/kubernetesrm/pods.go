...
func (p *pods) CreateQuota(quota int, namespaceName string) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.handleCreateQuotaRequest(quota, namespaceName)
}
...
func (p *pods) handleCreateQuotaRequest(quota int, namespaceName string) error {
	var k8sDeterminedLabel = map[string]string{determinedLabel: namespaceName}
