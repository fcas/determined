...
func (a *apiServer) setResourceQuota(ctx context.Context, req *apiv1.SetResourceQuotaRequest, tx *bun.Tx, w *model.Workspace) (*apiv1.SetResourceQuotaResponse, error) {
	...
	// namespaceName, err := generateNamespaceName(w.Name)
	err = a.m.rm.CreateQuota(int(req.Quota), wsns.NamespaceName, req.ClusterName)
	if err != nil {
